            ...
            if(args != null && args.isNotEmpty()) {
                ...
                if (chance > 100) {
                    if (user is Player) {
                        ...
                        getLogger().info("[ BoneTwerker ] Chance cannot be greater than 100!")
                    }
                    return true
                }
                ...
                if (user is Player) {
                    ...
                    getLogger().info("[ BoneTwerker ] Chance set to $chance")
                }
            } else {

                // Display current chance values
                if (user is Player) {
                    user.sendMessage("§f[ §aBoneTwerker §f] Chance set to §e" + mainInstance.config.get("chance"))
                } else {
                    getLogger().info("[ BoneTwerker ] Chance set to " + mainInstance.config.get("chance"))
                }
            }
