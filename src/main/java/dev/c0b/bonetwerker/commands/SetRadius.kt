        ...
        if (user.hasPermission("BoneTwerker.setradius")) {
            ...
            if (args != null && args.isNotEmpty()) {
                ...
                if (radius >= 10.0) {
                    if (user is Player) {
                        user.sendMessage(
                            "§f[ §eBoneTwerker §f] A radius of §e$radius§f is quite high, its your funeral!"
                        )
                    } else {
                        PaperPluginLogger.getGlobal()
                            .info(
                                "[ BoneTwerker ] A radius of $radius is quite high, its your funeral!"
                            )
                    }
                }
                ...
                if (user is Player) {
                    ...
                    PaperPluginLogger.getGlobal().info("[ BoneTwerker ] Radius set to $radius")
                }
            } else {
                // Display current radius values
                if (user is Player) {
                    user.sendMessage(
                        "§f[ §aBoneTwerker §f] Radius set to §e" + mainInstance.config.get("radius")
                    )
                } else {
                    PaperPluginLogger.getGlobal()
                        .info("[ BoneTwerker ] Radius set to " + mainInstance.config.get("radius"))
                }
            }
        } else {
            user.sendMessage(
